...
### POST `/api/game/squat/redeem`

将当前用户的深蹲次数按 `1 次 = 1 金币` 兑换为积分，并落一条“深蹲兑换”类型记录。
...
- 会新增一条 `game.type = 6` 的已结算记录。
- 会新增一条 `game_record.role = 5` 的用户积分明细。
- 只增加 `total_points`，不会增加 `total_games` 和 `win_count`。
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `gameType` | 是 | 游戏类型，1-平胡 2-自摸 3-一炮双响 4-一炮三响 5-相公 |
| `players` | 是 | 当前牌桌玩家 ID 列表；必须最终形成 4 人 |
...
- 记录者会额外得到随机奖励：1% 概率 `20` 分，99% 概率 `1` 分
